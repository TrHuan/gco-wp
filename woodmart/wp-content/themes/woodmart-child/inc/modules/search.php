...
if( ! function_exists( 'woodmart_search_form' ) ) {
	function woodmart_search_form( $args = array() ) {
		...
		switch ( $post_type ) {
			case 'product':
				$placeholder = esc_attr_x( 'Tìm kiếm sản phẩm', 'submit button', 'woodmart' );
				$description = esc_html__( 'Bắt đầu nhập để xem sản phẩm bạn đang tìm kiếm.', 'woodmart' );
			break;

			case 'portfolio':
				$placeholder = esc_attr_x( 'Tìm kiếm các dự án', 'submit button', 'woodmart' );
				$description = esc_html__( 'Bắt đầu nhập để xem các dự án bạn đang tìm kiếm.', 'woodmart' );
			break;
		
			default:
				$placeholder = esc_attr_x( 'Tìm kiếm các bài đăng', 'submit button', 'woodmart' );
				$description = esc_html__( 'Bắt đầu nhập để xem các bài đăng bạn đang tìm kiếm.', 'woodmart' );
			break;
		}
			...
			<div class="woodmart-search-<?php echo esc_attr( $type ); ?>">
				...
				<form role="search" method="get" class="searchform <?php echo esc_attr( $class ); ?>" action="<?php echo esc_url( home_url( '/' ) ); ?>" <?php echo ! empty( $data ) ? $data : ''; ?>>
					...
					<button type="submit" class="searchsubmit">
						<?php echo esc_attr_x( 'Tìm kiếm', 'submit button', 'woodmart' ); ?>
						<?php 
							if ( $icon_type == 'custom' ) {
		...
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			...
			<div class="search-by-category input-dropdown">
				<div class="input-dropdown-inner woodmart-scroll-content">
					...
					<a href="#" data-val="0"><?php esc_html_e('Chọn danh mục', 'woodmart'); ?></a>
					<div class="list-wrapper woodmart-scroll">
						<ul class="woodmart-scroll-content">
							<li style="display:none;"><a href="#" data-val="0"><?php esc_html_e('Chọn danh mục', 'woodmart'); ?></a></li>
							<?php
