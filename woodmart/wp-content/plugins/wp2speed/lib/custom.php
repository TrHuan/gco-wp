...
add_filter('hpp_allow_readyjs', function($ok, $js){
	if(hpp_in_str($js, ['new PerformanceObserver','enable_page_level_ads','window.dataLayer = window.dataLayer','adsbygoogle = window.adsbygoogle','var anr_onloadCallback']) /*|| hpp_in_str($js,['function ','function('])*/ ) return false;
	return $ok;
}, 10, 2);
...
add_filter('hpp_save_merge_file', function($text, $file) {
	...
	if(strpos($file,'.js')!==false) {
		//if(!empty($GLOBALS['jq'])) $text = $GLOBALS['jq']."\n".$text;
...
if(hw_config('server_cache')) remove_action( 'template_redirect', 'redirect_canonical' );
...
/*add_action('revslider_add_li_data', function($slider, $slide){
	if(!isset($GLOBALS["revslider-first-img"])) echo 'data-bg="'. $urlSlideImage .'" class="lazy"';
},10, 2);*/

add_filter('hpp_allow_readyjs', function($pass, $js){
	if(hpp_in_str($js, ['function setREVStartSize','function revslider_showDoubleJqueryError'])) return false;
	return $pass;
},10, 2);

add_filter('revslider_fe_before_init_script', function($html, $slider, $html_id){
	return $html;
}, 10, 3);

//move to 
add_filter('hpp_inline_script', function($js){
	if(strpos($js,'setREVStartSize({')!==false) {
		$js=str_replace('setREVStartSize({','jQuery("rs-slide img").each(function(){this.setAttribute("src",this.getAttribute("data-src"));});setREVStartSize({',$js);
	}
	return $js;
});
/*
add_filter('hpp_disallow_lazyload', function($ok, $tag){
	if(strpos($tag, 'rev-slidebg')!==false) return true;
	return $ok;
}, 10, 2);

add_filter('hpp_disallow_lazyload_attr', function($ok, $attr){
	if(strpos($attr['class'], 'rev-slidebg')!==false) return true;
	return $ok;
}, 10, 2);
*/
