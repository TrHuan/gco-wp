		    ...
		    <div class="container">
		        <!-- <div class="row"> -->
			        ...
			        <div class="groups-method__buy mb-10s">
			        	<?php
							$link_cs = get_field('link_consulting_survey1', 'option');
							if( $link_cs ) {
			            ...
			            <form class="cart" action="<?php echo wc_get_cart_url(); ?>" method="post" enctype="multipart/form-data">
							...
							sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" 
						     class="%s button ajax_add_to_cart1 single_add_to_cart_button buy-btn__details blues-buys__btn" data_link_cart="'.wc_get_checkout_url().'"><p class="titles-md__alls fs-16s">Mua ngay</p> <p class="fs-15s">(Xem hàng, không mua không sao)</p></a>',
			            ...
			            <div class="row gutter-6">
			                ...
			                <div class="col-6">
			                    <div class="items-hotline__details">
			                        <a title="" href="tel:<?php the_sub_field('phone'); ?>" class="number-hotline__details fs-18s titles-md__alls color-growns"><?php the_sub_field('phone'); ?></a>
					...
					<div class="text-alls__pages mb-20s">
				        <div class="intros-text__alls contents-hiddens mb-20s">
				            <p class="fs-16s titles-text__alls"><?php echo __('Tổng quan'); ?></p>
				            <div class="data_more_less">
                                <div class="data_more_less_inner" data-height="400" data-increase-by="100%" >
                                    <div class="data_more_less_body">
                                        <?php the_content(); ?>
                                    </div>
                                </div>
                                <a href="javascript:void(0)" class="action_more more_less_action">Xem thêm tổng quan <i class="fa fa-angle-down"></i></a>
                                <a href="#overview" class="action_less more_less_action">Thu gọn <i class="fa fa-angle-up"></i></a>
                            </div>
				        </div>
				    ...
				    <div class="text-alls__pages mb-20s" style="width:100%">
					    <div class="intros-table__alls contents-hiddens mb-20s">
					        <p class="fs-18s titles-text__alls mb-20s"><?php echo __('Thông số kỹ thuật'); ?></p>
					        <div class="table-all__text table-parameter__prds">
								...
								<div class="data_more_less">
									...
									<a href="javascript:void(0)" class="action_more more_less_action">Xem thêm thông số kỹ thuật <i class="fa fa-angle-down"></i></a>
									<a href="#specifications" class="action_less more_less_action">Thu gọn <i class="fa fa-angle-up"></i></a>
								</div>
					...
					<div class="col-xl-3 col-lg-6 col-md-12 col-sm-12 col-12">
						<div class="summary entry-summary">
							...
							<form class="cart" action="<?php echo wc_get_cart_url(); ?>" method="post" enctype="multipart/form-data">
								...
								<?php echo apply_filters( 'woocommerce_loop_add_to_cart_link',
								sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" 
							     class="%s button ajax_add_to_cart1 single_add_to_cart_button" data_link_cart="'.wc_get_checkout_url().'">Mua ngay <span>(Xem hàng, không mua không sao)</span></a>',
								esc_url( $product->add_to_cart_url() ),
								esc_attr( $product->id ),
								esc_attr( $product->get_sku() ),
								$product->is_purchasable() ? 'add_to_cart_button' : '',
			            	...
			            	while( have_rows('product_details', 'option') ) { the_row();
			            		if( have_rows('why_buy_eu_kitchen') ) { ?>
									<div class="why-buy-eu">
										<label><?php echo __('Tại sao mua hàng tại Bếp Việt Đức?'); ?></label>
								...
								if ( $rating_count > 0 ) : ?>
									<div class="woocommerce-product-rating">
										<?php if ( comments_open() ) : ?>
											<p class="woocommerce-review-link" rel="nofollow"> (<?php printf( _n( '%s người đánh giá', '%s người đánh giá', $review_count, 'woocommerce' ), '<span class="count">' . esc_html( $review_count ) . '</span>' ); ?>)</p>
										<?php endif ?>
									</div>
								<?php endif; ?>
							</div>
