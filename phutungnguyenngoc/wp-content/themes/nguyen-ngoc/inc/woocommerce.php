...
function custom_override_checkout_fields( $fields ) { 
    ...
    $fields['billing']['billing_address_1'] = array(
        'label' => __('Địa chỉ', 'lth'),
        'placeholder' => _x('Địa chỉ (Số xx, Ngõ xx, ... Hà Nội)', 'placeholder', 'lth'),
        'priority' => 3,
        'required' => true,
        'class' => array('form-row-wide'),
        'clear' => true
    ...
    $fields['shipping']['billing_address_1'] = array(
        'label' => __('Địa chỉ', 'lth'),
        'placeholder' => _x('Địa chỉ (Số xx, Ngõ xx, ... Hà Nội)', 'placeholder', 'lth'),
        'priority' => 3,
        'required' => true,
...
function lth_bank_details( $order_id = '' ) {
    ...
    if ( ! empty( $bacs_accounts ) ) {
        ...
        echo '<div>';
        ?>
        <ul style="margin-bottom: 20px;" class="bank-title">
            <?php $i;
            foreach ( $bacs_accounts as $bacs_account ) { $i++;
                ...
                <li style="border: 1px solid #ebebeb;padding: 10px 15px;" data_tab="tab-bank-<?php echo $i; ?>" class="<?php if ($i == 1){echo 'active';} ?>">
                    <p><?php echo $bank_name;?></p>
                </li>
            <?php } ?>
        </ul>

        <ul style="margin-bottom: 20px;" class="bank-content">
            <?php $j;
            foreach ( $bacs_accounts as $bacs_account ) { $j++;
                ...
                ?>
                <!-- <td style="width: 200px;border: 1px solid #ebebeb;padding: 6px 10px;"><?php if($icon):?><img src="<?php echo $icon;?>" alt=""/><?php endif;?></td> -->
                <li style="border: 1px solid #ebebeb;padding: 10px 15px;" class="tab-bank-<?php echo $j; ?> <?php if ($j == 1){echo 'active';} ?>">
...
function get_productloadmore() {
    $offset = isset($_POST['offset']) ? (int)$_POST['offset'] : 0; // lấy dữ liệu phía client gởi
    $getposts = new WP_query(); 
    $getposts->query('post_type=product&post_status=publish&orderby=modified&order=DESC&showposts=2&offset='.$offset);
