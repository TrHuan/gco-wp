...
if ( ! function_exists( 'woosg_update_checker' ) ) {
	...
	add_action( 'admin_notices', 'woosg_update_notice', 99 );

	function woosg_update_checker() {
		if ( $key = wpc_get_update_key( 13064 ) ) {
			PucFactory::buildUpdateChecker( 'https://api.wpclever.net/update/' . $key . '.json', WOOSG_FILE, plugin_basename( WOOSG_DIR ) );
		}
	}

	function woosg_update_notice() {
		if ( apply_filters( 'wpc_dismiss_notices', false ) ) {
			return;
		}

		if ( get_option( 'wpc_dismiss_notice_woosg_update' ) ) {
			return;
		}

		if ( ! wpc_get_update_key( 13064 ) ) {
			?>
            <div data-dismissible="woosg_update" class="wpc-notice notice notice-warning is-dismissible">
                <p>Please verify <a href="<?php echo admin_url( 'admin.php?page=wpclever-keys' ); ?>">License Key</a> of
                    <strong>WPC Grouped Product</strong> to enjoy unlimited update release and
