...
if ( ! class_exists( 'WPCleverWoosg' ) ) {
	class WPCleverWoosg {
		...
		function __construct() {
			...
			add_action( 'wp_ajax_woosg_add_text', [ $this, 'ajax_add_text' ] );
			...
			add_action( 'before_woocommerce_init', function () {
				if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
					\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', WOOSG_FILE );
				}
			} );
		}
					...
					<?php } elseif ( $active_tab === 'settings' ) {
                        ...
                        <form method="post" action="options.php">
                            <table class="form-table">
                                ...
                                <tr>
                                    <td colspan="2">
                                        To display custom engaging real-time messages on any wished positions,
                                        please install <a
                                                href="https://wordpress.org/plugins/wpc-smart-messages/"
                                                target="_blank">WPC Smart Messages for WooCommerce</a> plugin.
                                        It's free and
                        ...
                        <td>
                            ...
                            <!--
                                    <a href="https://wpclever.net/downloads/grouped-product?utm_source=pro&utm_medium=woosg&utm_campaign=wporg"
                                       target="_blank" class="woosg_add_txt"
		...
		function product_data_li( $product, $qty = 0, $search = false ) {
			...
			echo '<li class="' . esc_attr( $product_class ) . '" data-id="' . esc_attr( $product_id ) . '" data-price="' . esc_attr( $product->get_price() ) . '">' . $hidden_input . '<span class="move"></span><span class="qty hint--right" aria-label="' . esc_html__( 'Default quantity', 'wpc-grouped-product' ) . '">' . $qty_input . '</span><span class="img">' . $product->get_image( array(
					30,
					30
				) ) . '</span><span class="data"><span class="name">' . strip_tags( $product->get_name() ) . '</span> <span class="info">' . $product->get_price_html() . '</span> ' . ( $product->is_sold_individually() ? '<span class="info">sold individually</span> ' : '' ) . '</span> <span class="type"><a href="' . get_edit_post_link( $product_id ) . '" target="_blank">' . esc_attr( $product->get_type() ) . '<br/>#' . esc_attr( $product_id ) . '</a></span> ' . $remove_btn . '</li>';
		}

		function text_data_li( $data = array() ) {
			$key  = uniqid();
			$data = array_merge( array( 'type' => 'h1', 'text' => '' ), $data );
                ...
                <div class="woosg-table woosg-products"
					...
					foreach ( $items as $item ) {
						if ( ! empty( $item['id'] ) ) {
							...
							$item_regular_price = apply_filters( 'woosg_item_regular_price', wc_get_price_to_display( $product, array( 'price' => $product->get_regular_price() ) ), $product );
                            ...
                            <div class="<?php echo esc_attr( apply_filters( 'woosg_item_class', $item_class, $product, $global_product ) ); ?>"
								...
								if ( $show_thumb ) {
                                    ...
                                    <div class="woosg-thumb">
                                        ...
                                        <div class="woosg-thumb-ori">
											<?php echo wp_kses( apply_filters( 'woosg_item_thumbnail', $product->get_image( self::$image_size ), $product, $global_product, $order ), array(
												'img' => array(
													'width'       => array(),
													'height'      => array(),
													'sizes'       => array(),
                                ...
                                <div class="woosg-title">
									...
									echo wp_kses( apply_filters( 'woosg_item_name', $item_name, $product, $order ), array(
										'a'    => array(
											'class'   => array(),
											'data-id' => array(),
											'href'    => array(),
			...
			if ( ! empty( $ids ) ) {
				if ( is_array( $ids ) ) {
					...
					foreach ( $ids as $item ) {
						...
						if ( ! empty( $item['sku'] ) && ( $new_id = wc_get_product_id_by_sku( $item['sku'] ) ) ) {
		...
		function wpcsm_locations( $locations ) {
			$locations['WPC Grouped Product'] = array(
