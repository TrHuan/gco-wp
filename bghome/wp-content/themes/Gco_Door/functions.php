 ...
 * widget tạo bài viết mới
 */
class Sidebar_Product extends WP_Widget {
    function __construct() {
        parent::__construct(
            'sidebar_product',
            'Sản phẩm theo danh mục',
            array( 'description'  =>  'Widget hiển thị sản phẩm mới theo danh mục' )
        );
    }
    function form( $instance ) {
        $default = array(
        ...
        $title = esc_attr($instance['title']);
        $select = esc_attr($instance['select']);
        $product_number = esc_attr($instance['product_number']);
        $addclass = esc_attr($instance['addclass']);
        echo '<p>Nhập tiêu đề <input type="text" id="'.$this->get_field_id('title').'" class="widefat" name="'.$this->get_field_name('title').'" value="'.$title.'"/></p>';
        echo '<p>Số lượng sản phẩm hiển thị <input type="number" class="widefat" name="'.$this->get_field_name('product_number').'" value="'.$product_number.'" placeholder="'.$product_number.'" max="30" /></p>';
        ?>
    <!-- Category Select Menu -->   
    <p>
        <select id="<?php echo $this->get_field_id('select'); ?>" name="<?php echo $this->get_field_name('select'); ?>" class="widefat" style="width:100%;">
    ...
    <select name="<?php echo $this->get_field_name('addclass'); ?>" id="<?php echo $this->get_field_id('addclass'); ?>">
        <option <?php selected('product-style-1', $instance['addclass']); ?> value="product-style-1">Style 1 Image Left</option>
        <option <?php selected('product-style-2', $instance['addclass']); ?> value="product-style-2">Style 2 Image Right</option>
        <option <?php selected('product-style-3', $instance['addclass']); ?> value="product-style-3">Style 3 Image Center</option>
        <option <?php selected('product-style-4', $instance['addclass']); ?> value="product-style-4">Style 4 Image Top</option>
    </select>
        <?php
    }
    function update( $new_instance, $old_instance ) {
    ...
    function widget( $args, $instance ) {
        ...
        echo $before_widget;
        echo $before_title.$title.$after_title;
        $sidebar_product = new WP_Query(array(
            'post_type'=>'product',
            'post_status'=>'publish',
            'tax_query' => array(
                array(
        ...
        if ($sidebar_product->have_posts()):
            echo '<ul class="sidebar-product '.$addclass.'">';
            $i=1;while( $sidebar_product->have_posts() ) :
                $sidebar_product->the_post(); ?>
                <?php if($i==1): ?>
                <li>
                  <div class="box-sidebarproduct">
                   <div class="img-sidebarproduct">
                       <a href="<?php the_permalink() ;?>"> 
                           <?php the_post_thumbnail("medium",array( "title" => get_the_title(),"alt" => get_the_title() ));?>
                       </a>
                       <div class="details-sidebarproduct">
                          <?php global $product;
                   ...
                   <div class="info-sidebarproduct">
                      <a class="name-sidebarproduct" href="<?php the_permalink() ;?>" title=""><?php the_title() ;?></a>
                      <p class="price-sidebarproduct"><span><?php global $product;echo $product->get_price_html(); ?></span></p>
                   </div>
                  </div>                            
                </li>
                <?php else: ?>
...
add_action( 'widgets_init', 'create_sidebarproduct_widget' );
function create_sidebarproduct_widget() {
    register_widget('Sidebar_Product');
}
    ...
    function widget( $args, $instance ) {
        ...
        <?php if($address) { ?> 
          <li class="address-widget_contact">
            <i class="fal fa-map-marker-alt"></i><p><span>Địa chỉ: </span><?php echo $address;?></p>
          </li>
        <?php } ?>
        <?php if($address2) { ?> 
          <li class="address-widget_contact">
            <i class="fal fa-map-marker-alt"></i><p><span>Địa chỉ: </span><?php echo $address2;?></p>
          </li>
        <?php } ?>
        <?php if($hotline) { ?> 
          <li class="hotline-widget_contact">
            <i class="fal fa-phone"></i><p><span>Hotline: </span><a href="tel:<?php echo $hotline;?>"><?php echo $hotline;?></a></p>
          </li>
        <?php } ?>
        <?php if($email) { ?> 
          <li class="email-widget_contact">
            <i class="fal fa-envelope"></i><p><span>Email: </span><a href="mailto:<?php echo $email;?>"><?php echo $email;?></a></p>
          </li>
        <?php } ?>
        <!--<?php if($website) { ?> 
...
function woo_tab_instructions_products() {
  $instruction_products = get_field('instruction_products');

  echo '<div class="content-instructions_products">'.$instruction_products.'</div>';
}
...
function add_custom_addtocart_and_checkout() {
    ...
    jQuery(document).ready(function($) {
        ...
        setTimeout( function(){
            if( $(vid).val() != '' ){
                $(button).attr('href', url + $(vid).val() + '&quantity=' + $(qty).val() + '&variation_id=' + $(vid).val() + '&attribute_pa_kich-thuoc=' + $(tip).data('value'));
            }
        }, 300 );
        ...
        $(qty).on('input change', function() {
            if( $(vid).val() != '' ){
                $(button).attr('href', url + $(vid).val() + '&quantity=' + $(this).val() + '&variation_id=' + $(vid).val() + '&attribute_pa_kich-thuoc=' + $(tip).data('value'));
            }
        });
        ...
        $('.variations_form').on('change blur', 'table.variations li.selected', function() {
            if( $(vid).val() != '' ){
                $(button).attr('href', url + $(vid).val() + '&quantity=' + $(qty).val() + '&variation_id=' + $(vid).val() + '&attribute_pa_kich-thuoc=' + $(this).data('value'));
            }
        });
    });
    </script>
