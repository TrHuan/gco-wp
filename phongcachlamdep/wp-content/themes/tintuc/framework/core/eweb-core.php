...
/*-----------------------------------------------------------------------------------*/
	if ( ! function_exists( 'eweb_truncate_title' ) ){
		function eweb_truncate_title($amount,$echo=true,$post='') {
			global $post;
			...
			if ($echo_out == '...') $truncate = mb_substr($truncate, 0, strrpos(mb_substr($truncate, 0, $amount), ' '));
			else $truncate = mb_substr($truncate, 0, $amount);
...
/*-----------------------------------------------------------------------------------*/
	if ( ! function_exists( 'eweb_truncate_description' ) ){
		function eweb_truncate_description($amount,$echo=true,$post='') {
			global $post;
			$truncate = $post->post_content;
			//var_dump($truncate);die;
			$truncate = preg_replace('@\[caption[^\]]*?\].*?\[\/caption]@si', '', $truncate);
...
function eweb_custom_admin_footer() {
	echo '<a href="http://e-web.vn" title="Thiết kế website, thiết kế web wordpress theo chuẩn" target="_blank">Thiết kế website e-web.vn</a>';
}
