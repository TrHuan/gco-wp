...
class eweb_post_by_cate extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array(
            'classname' => 'md1',
            'description' => 'Bài viết theo chuyên mục (Content Home)'
        );
        parent::__construct('eweb_post_by_cate', 'EW: Bài viết theo chuyên mục', $widget_ops);
    }
    ...
    function widget($args, $instance) {
		...
		<?php echo $before_widget;?>
            <div class="subcat">
                ...
                <label class="select-child pull-right">
		            <select class="pro-subcat" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
		            	<?php
						$taxonomy_name = 'category';
						$termchildren = get_term_children( $cat_id, $taxonomy_name );
		            	echo '<option value="">Xem thêm &raquo;</option>';
						foreach ( $termchildren as $child ) {
							$term = get_term_by( 'id', $child, $taxonomy_name );
							echo '<option value="' . get_term_link( $child, $taxonomy_name ) . '">' . $term->name . '</option>';
						}?>
		            </select>
	        	</label>
            </div>
				    ...
				    while($my_query->have_posts()):$my_query->the_post();
				    	if($i==1){
				    		...
				    		<li <?php post_class($class);?>>
		                        ...
		                        <p><?php echo eweb_truncate_description('150');?></p>
		                    </li>
                    	<?php }else{?>
		                    <li>
