    ...
    function my_wp_nav_menu_objects( $items, $args ) { 
        foreach( $items as $item ) {
            ...
            if( $icon ) {                
                ...
                $item->title .= ' <i class="icofont-simple-down icon"></i>';
            }
...
function lth_sidebar_register() {
    ...
    register_sidebar (
        array (
            'name' => __('Chi Tiết Đội Tàu'),
            'id'        => 'sidebar_single_doi_tau',
            'before_widget' => '<div class="sidebar-box sidebar-doi-tau">',
            'after_widget' => '</div>',
            'before_title' => '<h2>',
            'after_title' => '</h2>',
        )
...
function reorder_my_cpt( $q ) {
  ...
  if ( $s->base === 'edit' && $s->post_type === 'doi-tau' ) {
    $q->set('orderby', 'ID');
    $q->set('order', 'DESC');
  }
}
