...
class WooCommerce_Widget_Currency_Converter extends WP_Widget {
	...
	public function __construct() {
		parent::__construct( $this->widget_id_base, esc_html__( 'WooCommerce Currency Converter', 'maxshop' ), array(
			'classname'   => 'widget_currency_converter',
			'description' =>  esc_html__( 'Allow users to choose a currency for prices to be displayed in.', 'maxshop' )
		) );
	}
	...
	public function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'maxshop' ) ?></label>
			<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" value="<?php if ( isset( $instance['title'] ) ) echo esc_attr( $instance['title'] ); else echo esc_html__( 'Currency converter', 'maxshop' ); ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'currency_codes' ); ?>"><?php esc_html_e( 'Currency codes:', 'maxshop' ); ?> <small>(<?php esc_html_e( '1 per line', 'maxshop' ) ?>)</small></label>
			...
			<p class="description"><?php esc_html_e( "Use * to control how the amounts and currency symbols are displayed. Example: SEK* becomes 999kr. USD * becomes 999 $. If you omit * and just provide the currency (USD, EUR), WooCommerce's default currency position will be used.", 'maxshop' ); ?></p>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'currency_display' ); ?>"><?php esc_html_e( 'Currency Display Mode:', 'maxshop' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'currency_display' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'currency_display' ) ); ?>">
				<option value="" <?php selected( $instance['currency_display'], '' ) ?>><?php esc_html_e( 'Buttons', 'maxshop' ); ?></option>
				<option value="select" <?php selected( $instance['currency_display'], 'select' ) ?>><?php esc_html_e( 'Select Box', 'maxshop' ); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'message' ); ?>"><?php esc_html_e( 'Widget message:', 'maxshop' ) ?></label>
			<textarea class="widefat" rows="5" cols="20" name="<?php echo $this->get_field_name( 'message' ); ?>" id="<?php echo $this->get_field_id( 'message' ); ?>"><?php if ( isset ( $instance['message'] ) ) echo esc_attr( $instance['message'] ); else esc_html_e( "Currency conversions are estimated and should be used for informational purposes only.", 'maxshop' ); ?></textarea>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'show_symbols' ); ?>"><?php esc_html_e( 'Show currency symbols in widget:', 'maxshop' ) ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_reset' ); ?>"><?php esc_html_e( 'Show reset link:', 'maxshop' ) ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'disable_location' ); ?>"><?php esc_html_e( 'Disable location detection:', 'maxshop' ) ?></label>
			...
			<p class="description"><?php esc_html_e( "The currency converter widget will default to the currency for a user's currenct location. Check this box to disable location detection and default to the store's currency.", 'maxshop' ); ?></p>
		</p>
		<?php
	}
}
