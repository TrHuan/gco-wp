...
function ya_title() {
	if (is_home()) {
		if (get_option('page_for_posts', true)) {
			...
			esc_html_e('Latest Posts', 'maxshop');
		}
	} elseif (is_archive()) {
...
class YA_Breadcrumbs {
	...
	function breadcrumb( $before = '', $after = '', $display = true ) {
		$options = array('breadcrumbs-home' => esc_html__( 'Trang chủ', 'maxshop' ), 'breadcrumbs-blog-remove' => false, 'post_types-post-maintax' => '0');
		...
		$links = array(
			array(
				'url'  => get_home_url(),
				'text' => ( isset( $options['breadcrumbs-home'] ) && $options['breadcrumbs-home'] != '' ) ? $options['breadcrumbs-home'] : esc_html__( 'Trang chủ', 'maxshop' )
			)
		);
		...
		if ( ( $on_front == "page" && is_front_page() ) || ( $on_front == "posts" && is_home() ) ) {
			...
			if ( is_post_type_archive() ) {
				...
				$bc = esc_html__( 'Lưu trữ cho', 'maxshop' );
				...
				$bc = esc_html__( 'Bạn tìm kiếm cho', 'maxshop' );
				$links[] = array( 'text' => $bc . ' "' . esc_html( get_search_query() ) . '"' );
			} elseif ( is_404() ) {
				$crumb404 = esc_html__( 'Lỗi 404 - Không tìm thấy trang', 'maxshop' );
