...
function ya_product_thumbnail( $size = 'shop_catalog', $placeholder_width = 0, $placeholder_height = 0  ) {
	...
	if ( has_post_thumbnail( $post->ID ) ){
		if( $attachment_image ){
			...
			$html .= (get_the_post_thumbnail( $post->ID, $size )) ? get_the_post_thumbnail( $post->ID, $size ): '<img src="'.get_template_directory_uri().'/assets/img/placeholder/'.$size.'.png" alt="Placeholder">';
...
function ya_banner_listing(){	
	...
	if( '' === $banner_enable ){
		...
		$html .= '<img src="'. esc_url( $banner_listing ) .'" alt="No thumb"/>';
		$html .= ( $link_banner != '' ) ? '</a>': '';
	}else{
		global $wp_query;
		...
		if( !is_shop() ) {
			...
			if( $image ) {
				$html .= ( $link_banner != '' ) ? '<a href="'.esc_url($link_banner).'">': '';
				$html .= '<img src="'. esc_url( $image ) .'" alt="No thumb"/>';
				$html .= ( $link_banner != '' ) ? '</a>': '';
			}else{
				$html .= ( $link_banner != '' ) ? '<a href="'.esc_url($link_banner).'">': '';
				$html .= '<img src="'. esc_url( $banner_listing ) .'" alt="No thumb"/>';
...
?>
		<div class="item-brand">
			...
			?>
				<a href="<?php echo get_term_link( $term->term_id, 'product_brand' ); ?>"><img src="<?php echo wp_get_attachment_thumb_url( $thumbnail_id ); ?>" alt="No thumb" title="<?php echo esc_attr( $term->name ); ?>"/></a>				
			<?php 
					}else{
			?>
