...
// 					);
		...
		public function _options_form(){
			...
			foreach($this->sections as $k => $section){
				...
				if ( isset($section['fields']) && $this->getCpanelCheck($section['fields']) === true ){
					$icon = (!isset($section['icon']))?'<img src="'.$this->url.'img/glyphicons/glyphicons_019_cogwheel.png" alt="No thumb"/> ':'<img src="'.esc_attr( $section['icon'] ).'" alt="No thumb" /> ';
		...
		function _options_page_html(){
			...
			foreach($this->sections as $k => $section){
				$icon = (!isset($section['icon']))?'<img src="'.$this->url.'img/glyphicons/glyphicons_019_cogwheel.png" alt="No thumb"/> ':'<img src="'.$section['icon'].'" alt="No thumb"/> ';
			...
			do_action('ya-opts-after-section-menu-items-'.$this->args['opt_name'], $this);
						...
						//if(true === $this->args['show_import_export']){
							...
							echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="ya-opts-group-tab-link-a" data-rel="import_export_default"><img src="'.$this->url.'options/img/glyphicons/glyphicons_082_roundabout.png" alt="No thumb"/> <span>'.__('Import / Export', 'maxshop').'</span></a>';
							echo '</li>';
							// echo '<li class="divide">&nbsp;</li>';
						// }//if
						...
						// foreach($this->extra_tabs as $k => $tab){
							// $icon = (!isset($tab['icon']))?'<img src="'.$this->url.'img/glyphicons/glyphicons_019_cogwheel.png" alt="No thumb"/> ':'<img src="'.$tab['icon'].'" alt="No thumb"/> ';
							// echo '<li id="'.$k.'_section_group_li" class="ya-opts-group-tab-link-li">';
			...
			if(isset($this->args['share_icons'])){
				...
				foreach($this->args['share_icons'] as $link){
					echo '<a href="'.esc_url( $link['link'] ).'" title="'.esc_attr( $link['title'] ).'" target="_blank"><img src="'.esc_attr( $link['img'] ).'" alt="No thumb"/></a>';
				}
				echo '</div>';
			}
