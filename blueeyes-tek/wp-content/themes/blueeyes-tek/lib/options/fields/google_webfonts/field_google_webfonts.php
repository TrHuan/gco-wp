...
class YA_Options_google_webfonts extends YA_Options{	
	...
	*/
	function render(){
		...
		echo '<option value="" selected>'.esc_html__( 'Select Font', 'maxshop' ).'</option>';
		
		foreach($this->field['fonts'] as $cut){
			echo '<option value="'. esc_attr( $cut ).'" '.selected($this->value, $cut, false).'>'. $cut .'</option>';
		}
		echo '</select>';
		echo '<p class="description">'.sprintf( __('Please <a href="%s" target="_blank">browse the directory</a> to preview a font, then select your choice below.', 'maxshop') , esc_url( 'http://www.google.com/webfonts' ) ).'</p>';
