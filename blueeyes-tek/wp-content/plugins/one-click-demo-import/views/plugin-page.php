...
<div class="ocdi  wrap  about-wrap">
	...
	<?php if ( empty( $this->import_files ) ) : ?>
		...
		<div class="ocdi__file-upload-container">
			...
			<?php if ( class_exists( 'OptionsFramework' ) ) : ?>
			<div class="ocdi__file-upload">
				<h3><label for="options-file-upload"><?php esc_html_e( 'Choose a JSON file for Options import:', 'pt-ocdi' ); ?></label></h3>
				<input id="ocdi__options-file-upload" type="file" name="options-file-upload">
				<div>
					<label for="options-option-name" class="ocdi__options-option-name-label"><?php esc_html_e( 'Enter the Options option name:', 'pt-ocdi' ); ?></label>
					<input id="ocdi__options-option-name" type="text" name="options-option-name">
				</div>
			</div>
			<?php endif; ?>
		</div>
					...
					<div class="ocdi__gl-item js-ocdi-gl-item" data-categories="<?php echo esc_attr( Helpers::get_demo_import_item_categories( $import_file ) ); ?>" data-name="<?php echo esc_attr( strtolower( $import_file['import_file_name'] ) ); ?>">
						...
						<div class="ocdi__gl-item-footer<?php echo ! empty( $import_file['preview_url'] ) ? '  ocdi__gl-item-footer--with-preview' : ''; ?>">
							...
							<div class="footer-bottom">
								<select name="layout-demo" class="select-import-type">
									<option value="0"><?php echo esc_html__( 'Import All Content', 'pt-ocdi' ); ?></option>
									<option value="1"><?php echo esc_html__( 'Import All Pages Only', 'pt-ocdi' ); ?></option>
									<option value="2"><?php echo esc_html__( 'Import All Pages and Menu', 'pt-ocdi' ); ?></option>
								</select>
								<button class="ocdi__gl-item-button  button  button-primary  js-ocdi-gl-import-data" value="<?php echo esc_attr( $index ); ?>" data-layout-demo="0"><?php esc_html_e( 'Import', 'pt-ocdi' ); ?></button>
