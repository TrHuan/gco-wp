...
class Helpers {
	 ...
	 */
	public static function download_import_files( $import_file_info, $importtype ) {
		...
		if( $importtype == 1 ){
			if ( file_exists( $import_file_info['local_import_page_file'] ) ) {
				$downloaded_files['content'] = $import_file_info['local_import_page_file'];
			}
		}elseif( $importtype == 2 ){
			if ( file_exists( $import_file_info['local_import_pagemenu_file'] ) ) {
				$downloaded_files['content'] = $import_file_info['local_import_pagemenu_file'];
			}
		}
		...
		// ----- Set Options file paths -----
		// Get Options import file as well. If defined!
		if ( ! empty( $import_file_info['import_options'] ) && is_array( $import_file_info['import_options'] ) ) {
			$options_items = array();

			// Setup filename paths to save the Options content.
			foreach ( $import_file_info['import_options'] as $index => $options_item ) {
				$options_filename = apply_filters( 'pt-ocdi/downloaded_options_file_prefix', 'demo-options-import-file_' ) . $index . '-' . self::$demo_import_start_time . apply_filters( 'pt-ocdi/downloaded_options_file_suffix_and_file_extension', '.json' );

				// Download the Options import file.
				$file_path = $downloader->download_file( $options_item['file_url'], $options_filename );
			...
			// Download the Options import file.
			$downloaded_files['options'] = $options_items;
		}
		else if ( ! empty( $import_file_info['local_import_options'] ) ) {

			$options_items = array();

			// Setup filename paths to save the Options content.
			foreach ( $import_file_info['local_import_options'] as $options_item ) {
				if ( file_exists( $options_item['file_path'] ) ) {
		...
		// Download Revolution Slider
		if ( empty( $import_file_info['import_revslider'] ) ) {
			if ( file_exists( $import_file_info['local_import_revslider'] ) ) {
				$downloaded_files['revslider'] = $import_file_info['local_import_revslider'];
			}
		}
		else{
			$revslider_filename = apply_filters( 'pt-ocdi/downloaded_revslider_file_prefix', 'demo-revslider-import-file_' ) . self::$demo_import_start_time . apply_filters( 'pt-ocdi/downloaded_revslider_file_suffix_and_file_extension', '.zip' );
			$downloaded_files['revslider'] = $downloader->download_file( $import_file_info['import_revslider'], $revslider_filename );
		}
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		$options_file_info       = wp_handle_upload( $_FILES['options_file'], $upload_overrides );
		...
		// Process Options import file.
		if ( $options_file_info && ! isset( $options_file_info['error'] ) ) {
			if ( isset( $_POST['options_option_name'] ) && empty( $_POST['options_option_name'] ) ) {
				// Write error to log file and send an AJAX response with the error.
				self::log_error_and_send_ajax_response(
					esc_html__( 'Missing Options option name! Please also enter the Options option name!', 'pt-ocdi' ),
					$log_file_path,
					esc_html__( 'Upload files', 'pt-ocdi' )
				);
			}

			// Set uploaded Options file.
			$selected_import_files['options'] = array(
				array(
					'option_name' => $_POST['options_option_name'],
					'file_path'   => $options_file_info['file'],
				),
			);
		}
		else {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Options file was not uploaded. Error: %s', 'pt-ocdi' ),
					$options_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files' , 'pt-ocdi' )
			);
	...
	public static function import_file_info( $selected_import_files ) {
		...
		if ( ! empty( $selected_import_files['options'] ) ) {
			$options_file_string = array_reduce( $selected_import_files['options'], function( $string, $item ) {
		...
		sprintf(
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s%1$sOptions files:%1$s%6$s', 'pt-ocdi' ),
			PHP_EOL,
			get_site_url(),
			...
			empty( $options_file_string ) ? esc_html__( 'not defined!', 'pt-ocdi' ) : $options_file_string
		);
	}
