...
class OneClickDemoImport {
	...
	protected function __construct() {
		...
		add_action( 'wp_ajax_nopriv_ocdi_import_demo_data', array( $this, 'import_demo_data_ajax_callback' ) );
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			$import_type =  empty( $_POST['importtype'] ) ? 0 : absint( $_POST['importtype'] );
			...
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				$this->selected_import_files = Helpers::download_import_files( $this->import_files[ $this->selected_index ], $import_type );
		 ...
		 */
		$args = '';
		if ( ! empty( $this->selected_import_files['content'] ) && $this->selected_import_files['content'] != get_option( 'one_click_options' ) ) {
			$arg.= $this->selected_import_files['content'];			
			$this->append_to_frontend_error_messages( $this->importer->import_content( $this->selected_import_files['content'] ) );
			update_option( 'one_click_options', $arg );
		}
		 ...
		 * 3 - Import Options data (with priority 30).
		 */
