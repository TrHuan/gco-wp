...
if ( ! function_exists( 'apexclinic_posted_on' ) ) :
	...
	function apexclinic_posted_on() {
		...
		$published_on= apexclinic_global_var( 'prefix_date', '', false ).$published_on;
		if ( true != apexclinic_global_var( 'display_author_information', '', false ) ) {
             $author_image="";
        }
        if ( true != apexclinic_global_var( 'display_date', '', false ) ) {
        $published_on="";
        }
				printf(
					wp_kses_post(
