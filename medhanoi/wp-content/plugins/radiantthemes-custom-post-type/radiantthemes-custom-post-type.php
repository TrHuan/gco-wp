 ...
 * Description: Basic Custom Post Types. Custom Post Types include  Team, Clients, Our Story and Testimonials.
 * Version: 1.2.1
...
function team_build_meta_box( $post ) {
	...
	// retrieve the qualification current value.
	$current_qualification = get_post_meta( $post->ID, 'qualification', true );

	// retrieve the _team_facebook current value.
	$current_phone = get_post_meta( $post->ID, 'phone', true );

	// retrieve the _team_facebook current value.
	$current_email = get_post_meta( $post->ID, 'email', true );
	...
	$current_gplus = get_post_meta( $post->ID, 'gplus', true );

	// retrieve the _team_pinterest current value.
	$current_linkedin = get_post_meta( $post->ID, 'linkedin', true );
	
	// retrieve the schedule current value.
	$current_schedule = get_post_meta( $post->ID, 'schedule', true );
	
	$gpminvoice_group = get_post_meta($post->ID, 'customdata_group', true);

	?>
	<div class='inside'>
	    <h3><?php esc_html_e( 'Qualification', 'radiantthemes-custom-post-type' ); ?></h3>
		<p>
			<input type="text" class="widefat" name="qualification" value="<?php echo esc_html( $current_qualification ); ?>" />
		</p>
		...
		<h3><?php esc_html_e( 'Schedule', 'radiantthemes-custom-post-type' ); ?></h3>
    ...
    <tr>
      <td width="40%">
        <input type="text" class="widefat" placeholder="Day" name="TitleItem[]" value="<?php if($field['TitleItem'] != '') echo esc_attr( $field['TitleItem'] ); ?>" /></td> 
      <td width="40%">
        <input type="text" class="widefat" placeholder="Time" name="TitleDescription[]" value="<?php if($field['TitleDescription'] != '') echo esc_attr( $field['TitleDescription'] ); ?>" /></td>  
    ...
    <tr>
      <td> 
        <input type="text" placeholder="Day" title="Day" name="TitleItem[]" /></td>
      <td> 
        <input type="text" placeholder="Time" title="Time" name="TitleDescription[]" /></td>
      </td>
...
function team_save_meta_box_data( $post_id ) {
	...
	// store custom fields values
	// qualification.
	if ( isset( $_REQUEST['qualification'] ) ) { // Input var okay.
		update_post_meta( $post_id, 'qualification', sanitize_text_field( wp_unslash( $_POST['qualification'] ) ) ); // Input var okay.
	}
	// store custom fields values
	// phone link.
	if ( isset( $_REQUEST['phone'] ) ) { // Input var okay.
		update_post_meta( $post_id, 'phone', sanitize_text_field( wp_unslash( $_POST['phone'] ) ) ); // Input var okay.
	}
	// store custom fields values
	// email link.
	if ( isset( $_REQUEST['email'] ) ) { // Input var okay.
		update_post_meta( $post_id, 'email', sanitize_text_field( wp_unslash( $_POST['email'] ) ) ); // Input var okay.
	}
	// store custom fields values
	// facebook link.
	if ( isset( $_REQUEST['facebook'] ) ) { // Input var okay.
		update_post_meta( $post_id, 'facebook', sanitize_text_field( wp_unslash( $_POST['facebook'] ) ) ); // Input var okay.
	}
	// store custom fields values
	// twitter link.
	if ( isset( $_REQUEST['twitter'] ) ) { // Input var okay.
		update_post_meta( $post_id, 'twitter', sanitize_text_field( wp_unslash( $_POST['twitter'] ) ) ); // Input var okay.
	}
	// store custom fields values
	// gplus link.
	if ( isset( $_REQUEST['gplus'] ) ) { // Input var okay.
		update_post_meta( $post_id, 'gplus', sanitize_text_field( wp_unslash( $_POST['gplus'] ) ) ); // Input var okay.
	}
	// store custom fields values
	// pinterest link.
	if ( isset( $_REQUEST['linkedin'] ) ) { // Input var okay.
		update_post_meta( $post_id, 'linkedin', sanitize_text_field( wp_unslash( $_POST['linkedin'] ) ) ); // Input var okay.
	}
	// store custom fields values
	// store schedule
    ...
    $count = count( $invoiceItems );
     for ( $i = 0; $i < $count; $i++ ) {
        if ( $invoiceItems[$i] != '' ) :
            ...
            $new[$i]['TitleDescription'] = stripslashes( strip_tags( $prices[$i] ) );
        endif;
    }
...
/**
 * Add meta box for client
 *
 * @param post $post The post object.
 * @link https://codex.wordpress.org/Plugin_API/Action_Reference/add_meta_boxes
 */
function client_add_meta_boxes( $post ) {
	add_meta_box(
		'client_meta_box',
		__( 'CLIENT INFORMATION', 'radiantthemes-custom-post-type' ),
...
add_action( 'add_meta_boxes_client', 'client_add_meta_boxes' );
...
function client_build_meta_box( $post ) {
	// make sure the form request comes from WordPress.
	wp_nonce_field( basename( __FILE__ ), 'client_meta_box_nonce' );

	// retrieve the clientlink current value.
	$clientlink = get_post_meta( $post->ID, 'clientlink', true );
	?>
	<div class='inside'>
		<h3><?php esc_html_e( 'Client Link', 'radiantthemes-custom-post-type' ); ?></h3>
		<input type="text" class="widefat" name="clientlink" value="<?php echo esc_attr( $clientlink ); ?>" />
	</div>
<?php
}
...
function client_save_meta_box_data( $post_id ) {
	// verify meta box nonce.
	if ( ! isset( $_POST['client_meta_box_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['client_meta_box_nonce'] ), basename( __FILE__ ) ) ) { // Input var okay.
		return;
	}
	// return if autosave.
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
	...
	// store custom fields values
	// clientlink string.
	if ( isset( $_REQUEST['clientlink'] ) ) { // Input var okay.
		update_post_meta( $post_id, 'clientlink', sanitize_text_field( wp_unslash( $_POST['clientlink'] ) ) ); // Input var okay.
	}
}
