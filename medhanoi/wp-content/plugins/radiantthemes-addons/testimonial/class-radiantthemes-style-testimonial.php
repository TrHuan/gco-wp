...
if ( class_exists( 'WPBakeryShortCode' ) && ! class_exists( 'Radiantthemes_Style_Testimonial' ) ) {
	...
	class Radiantthemes_Style_Testimonial extends WPBakeryShortCode {
					...
					'params'      => array(
						array(
							...
							'value'      => array(
								...
								esc_html__( 'Style Nine (Bottom Left Image)', 'radiantthemes-addons' )   => 'nine',
							),
							'std'        => 'one',
						),
						array(
		...
		public function radiantthemes_testimonial_style_func( $atts, $content = null, $tag ) {
			...
			.testimonial.element-eight.' . $random_class . ' .testimonial-item > .holder > .main-holder > .testimonial-title-data cite{
					...
					if ( 'one' === $shortcode['testimonial_style'] ) {
						...
						$output             .= '<div class="holder matchHeight">';
						    $output         .= '<div class="main-holder">';
    							...
    							$output     .= '</div>';
    							$output     .= '<div class="testimonial-title-data">';
    							    $output .= '<cite><strong>' . esc_attr( get_the_title() ) . '</strong> ' . esc_attr( get_post_meta( get_the_ID(), '_testimonial_designation', true ) ) . '</cite>';
    							    $output .= '<blockquote>';
									$output .= '<p>' . esc_attr( get_the_content() ) . '</p>';
    								$output .= '</blockquote>';
    							$output     .= '</div>';
