...
if ( class_exists( 'WPBakeryShortCode' ) && ! class_exists( 'Radiantthemes_Style_Blog' ) ) {
	...
	class Radiantthemes_Style_Blog extends WPBakeryShortCode {
		...
		public function radiantthemes_blog_style_func( $atts, $content = null, $tag ) {
			...
			// ADD CUSTOM CSS.
			$custom_css = $shortcode['blog_color'] ? '.blog.element-two.' . $random_class . ' .blog-item > .holder > .category-display ul > li a,
			.blog.element-five.' . $random_class . ' .blog-item > .holder > .date,
            .blog.element-ten.' . $random_class . ' .blog-item > .holder > .data:before,
            .blog.element-twelve.' . $random_class . ' .blog-item > .holder:hover{
			    background-color: ' . $shortcode['blog_color'] . ' ;
			}' : '';

			$custom_css .= $shortcode['blog_color'] ? '.blog.element-two.' . $random_class . ' .blog-item > .holder > .data ul.post-meta li strong,
			.blog.element-five.' . $random_class . ' .blog-item > .holder .data .btn,
            .blog.element-six.' . $random_class . ' .blog-item > .holder .data .btn,
            .blog.element-ten.' . $random_class . ' .blog-item > .holder .data ul.meta li i,
            .blog.element-twelve.' . $random_class . ' .blog-item > .holder > .data > .author .date{
			    color: ' . $shortcode['blog_color'] . ' ;
			}' : '';

			wp_add_inline_style( 'radiantthemes_blog_' . $shortcode['style_variation'], $custom_css );
