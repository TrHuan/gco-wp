<?php
if ( ! class_exists( 'Shortcode_Heading' ) ) {
	class Shortcode_Heading extends Shortcode
	{
		/**
		 * Shortcode name.
		 *
		...
		{
			$atts = function_exists( 'vc_map_get_attributes' ) ? vc_map_get_attributes( 'custom_heading', $atts ) : $atts;
			extract( $atts );
			$css                        = '';
			$heading_reponsive = $styles = array();
			if ( isset( $atts['heading_reponsive'] ) ) {
				$heading_reponsive = vc_param_group_parse_atts( $atts['heading_reponsive'] );
			}
			if ( $heading_reponsive && count( $heading_reponsive ) > 0 ) {
				foreach ( $heading_reponsive as $item ) {
					$style = '';
					if ( isset( $item['responsive_font_container'] ) ) {
						$font_container_field          = WPBMap::getParam( 'custom_heading', 'heading_reponsive' );
		...
		public function getAttributes( $atts )
		{
			 ...
			 **/
			$google_fonts_field            = WPBMap::getParam( 'custom_heading', 'google_fonts' );
			$font_container_field          = WPBMap::getParam( 'custom_heading', 'font_container' );
			$font_container_obj            = new Vc_Font_Container();
		...
		static function getStyles( $el_class, $css, $google_fonts_data, $font_container_data, $atts )
		{
			 ...
			 * @since 4.3
			 */
			$css_class   = array( 'lth-custom-heading' );
			$css_class[] = $atts['el_class'];
			$css_class[] = $atts['custom_heading_custom_id'];
			$css_class[] = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, '', 'custom_heading', $atts );
		...
		public function output_html( $atts, $content = null )
		{
			...
			return apply_filters( 'Shortcode_Heading', $output, $atts, $content );
		}
	}

	new Shortcode_Heading();
}