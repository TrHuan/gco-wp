...
class sh_blog_shortcode {
	...
	function sh_blog_style_4 ( $the_query, $atts ) {
		...
		$html = '';
		$html .= '<div class="slick-slider slick-blogs">';
		while ( $the_query->have_posts() ) { $the_query->the_post();
	...
	function sh_general_post_html ( $post_class = array(), $atts = array(), $image_size = 'sh_thumb300x200' ) {
		...
		$html = '';
		$html .= '<div id="post-'. get_the_ID() .'" class=""><div class="post-inner">';
		...
		$html .= '<div class="entry-content">';
			...
			$html .= '<h3 class="entry-title content-name"><a href="'. get_permalink() .'" title="'. get_the_title() .'">'. get_the_title() .'</a></h3>';
			// Metadata
			if ( $hide_meta == '1' ) {
				$html .= '<div class="entry-meta content-date">';
			...
			if ( $hide_desc == '1' ) {
				$html .= '<div class="entry-description content-excerpt">'. get_the_content_limit( $number_character,' ' ) .'</div>';
			}
			// Check display view more button
			if ( $btn_viewmore == '1' ) {
				$html .= '<div class="content-button"><a href="'. get_permalink() .'" title="'. get_the_title() .'" class="view-more">'. $viewmore_text .' <i class="typo-icon typo-icon-angle-right icon"></i></a></div>';
			}
		$html .= '</div>';
		$html .= '</div></div>';
		return $html;
	...
	function sh_general_post_html_style_2 ( $post_class = array(), $atts = array(), $image_size = 'sh_thumb300x200' ) {
		...
		$html .= '<div class="entry-content">';
			...
			// Check display view more button
			if ( $btn_viewmore == '1' ) {
				$html .= '<div class="text-left"><a class="view-detail" href="'. get_permalink() .'" title="'. get_the_title() .'">'. $viewmore_text .' <i class="typo-icon typo-icon-angle-right icon"></i></a></div>';
			}
		$html .= '</div>';
		$html .= '</div></div>';
		return $html;
