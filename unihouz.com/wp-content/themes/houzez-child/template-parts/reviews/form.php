...
if( houzez_option('login_for_review', 0) == 1 && !is_user_logged_in() ) {
	...
	if( houzez_option('header_login') ) { 
        echo wp_kses(__( 'Xem lại danh sách <a class="hhh_login" href="#" data-toggle="modal" data-target="#login-register-form">đăng nhập</a>.', 'houzez' ), $allowed_html_array); 
        echo ' ';
    } else {
        echo wp_kses(__( 'Để xem lại danh sách này, vui lòng đăng nhập.', 'houzez' ), $allowed_html_array); 
    }

    if( houzez_option('header_register') ) { 
        echo wp_kses(__( 'Bạn chưa có tài khoản? <a class="hhh_regis" href="#" data-toggle="modal" data-target="#login-register-form">Đăng ký</a>', 'houzez' ), $allowed_html_array); 
    }
    echo '</div>';
