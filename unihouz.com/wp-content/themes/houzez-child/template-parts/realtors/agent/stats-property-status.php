...
foreach ($taxnonomies as $slug => $name) {
	$count = houzez_realtor_stats('property_status', 'fave_agents', get_the_ID(), $slug);
	if(!empty($count)) {
		$total_count[] = $count;
...
<div class="agent-profile-chart-wrap">
	<h2><?php echo wp_kses(__( 'Tình trạng <span>Bất động sản</span> ', 'houzez' ), houzez_allowed_html() ); ?></h2>
