...
if( isset( $_POST['user_profile_nonce_field'] ) 
  && wp_verify_nonce( $_POST['user_profile_nonce_field'], 'user_profile_nonce' ) ) {
    ...
    if ( !empty( $_POST['jop'] ) ) {
        update_user_meta( 
            $current_user->ID, 'jop', esc_attr( $_POST['jop'] ) 
        ); 
    }
			        		...
			        		<div class="author-box">
			        			<?php if($user_id == $author_id) { ?>
							        <form role="form" action="" id="user_profile" method="POST">
									    ...
									    <div class="form-group">
									        <label for="phone"><?php echo __('Jop'); ?></label>
									        <input type="text" class="form-control" id="jop" name="jop" placeholder="<?php echo __('Jop'); ?>" value="<?php the_author_meta( 'jop', $author_id ); ?>">
									    </div>
									    <div class="form-group">
