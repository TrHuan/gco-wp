...
// ajax load more
add_action('wp_ajax_postloadmore', 'get_postloadmore');
add_action('wp_ajax_nopriv_postloadmore', 'get_postloadmore');
function get_postloadmore() {
    $offset = isset($_POST['offset']) ? (int)$_POST['offset'] : 0; // lấy dữ liệu phái client gởi
    $getposts = new WP_query(); 
    $getposts->query('post_type=post&post_status=publish&showposts=5&offset='.$offset);
